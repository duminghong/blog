...
除了基本的`then()`和`catch()`方法外，Promise 还提供了其他一些实用的静态方法和实例方法，这些方法可以帮助我们更灵活地处理异步操作。

## Promise.resolve() {#Promise.resolve}

`Promise.resolve()`方法返回一个以给定值解析后的 Promise 对象。如果这个值是一个 Promise，那么将返回这个 Promise；如果这个值是 thenable（即带有`then`方法），Promise 会执行它的 then 方法；否则返回的 Promise 会以这个值兑现。
...
`Promise.all()`方法接收一个 Promise 可迭代对象（通常是数组），并返回一个新的 Promise。当所有的 Promise 都成功解决时，返回的 Promise 才会解决，解决值是一个包含所有 Promise 解决值的数组。只要有任何一个 Promise 被拒绝，返回的 Promise 就会被拒绝，拒绝原因是第一个被拒绝的 Promise 的拒绝原因。
...
## 实例方法：Promise.prototype.finally() {#Promise.prototype.finally}
...
## 实例方法：Promise.prototype.catch() {#Promise.prototype.catch}
...
## 实例方法：Promise.prototype.then() {#Promise.prototype.then}
