...
data: {
  ...
  runSteps: [
    ...
    },
    {
      ...
      createTask: {},
      codeNumbers: [23],
      result: "调用 **async1** 函数，进入函数执行，`async1()上下文`入栈<br>async1是一个async函数，返回一个Promise",
      console: "",
      runTime: 2000,
      deleteCallStack: []
    },
    ...
    },
    {
      ...
      createTask: {},
      codeNumbers: [3],
      result: "调用 `async2` 函数，`async2()上下文`入栈，<br>遇到 **await**`await`会暂停`async1`函数的执行，等待`async2`返回的**Promise**解决",
      console: "",
      runTime: 2000,
      deleteCallStack: []
    },
    ...
    },
    {
      ...
      codeNumbers: [8,9,10,11,12,13],
      result: "await等待的一定是一个Promise，所以会包装成`await Promise.resolve(延时器id)`，`Promise.resolve(延时器id)`已经完成，把后续代码推入微队列，如果没有后续，这把函数执行结束推到微队列<br>此时`async2`执行结束，`async2()上下文`出栈<br>创建任务 `async2执行结束` 到微队列",
      console: "",
      runTime: 2000,
      deleteCallStack: [7,9]
    },
    ...
    },
    {
      ...
      codeNumbers: [3],
      result: "执行微任务队列中的 `async2执行结束`，async2函数执行完毕，返回一个已解决的Promise，继续执行，把`console.log(2);`放入微任务队列<br>此时`async1`执行结束，`async1()上下文`出栈<br>创建任务 `async1-log(2)` 到微队列",
      console: "",
      runTime: 2000,
      deleteCallStack: [5,14]
    },
    {
      id: 15,
      type: "microtask",
    ...
    },
    {
      ...
      result: "执行延时任务队列中的 `setTimeout回调`，唯一一个被放入宏任务队列的任务，Promise.resolve().then回调会被放入微任务队列<br>创建任务 `async2-timeout-log(3)` 到微队列<br>执行后续代码`console.log(4);`<br>输出结果： 4",
      console: "4",
      runTime: 2000,
      deleteCallStack: [17]
    },
