  ...
  runSteps: [
    {
      ...
      createTask: {},
      codeNumbers: [1],
      result: "创建全局变量 **promise1**，调用Promise.resolve('success')，Promise.resolve内部执行，创建已fulfilled的Promise对象，赋值为 Promise {<fulfilled>: 'success'}",
      console: "",
      runTime: 2000
    },
    {
    ...
    },
    {
      ...
      codeNumbers: [3],
      result: '调用 **resolve(promise1)**，resolve检测到参数是Promise，触发 **promise2** 的状态解析，处理Promise状态吸收，返回一个pending状态的Promise对象<br>创建任务 `准备吸收` 到微队列',
      console: "",
      runTime: 2000
    },
    {
      ...
      taskName: "p2注册then回调",
      createTask: {},
      codeNumbers: [6,7,8,10,11,12,13,14],
    ...
    },
    {
      ...
      result: "注册then回调：`p1-then1` `p1-then2` `p1-then3`<br>因为 **promise1** 已fulfilled，准备执行第一个then回调 `p1-then1`",
      console: "",
      runTime: 2000
    },
    {
      ...
      result: "执行 **promise1** 的第一个then回调 `p1-then1`，创建任务 `log(4)` 到微队列，同步代码结束。",
      console: "",
      runTime: 2000
    },
    {
